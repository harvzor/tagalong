...
- [x] 4.1 Build and install the debug APK on the Poco X5; pick a video that was shot with GPS enabled; confirm the ProbeCard `location` row shows the expected coordinates (not `—`)
- [x] 4.2 Deny the `ACCESS_MEDIA_LOCATION` request; confirm the warning text appears and the file picker still opens; confirm the ProbeCard `location` row shows `—` and no crash occurs
- [x] 4.3 Grant the permission from device Settings and re-pick the same video; confirm the warning is gone and the `location` row shows correctly
